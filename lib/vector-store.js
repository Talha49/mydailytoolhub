 ...
 * @returns {Promise<Object[]>} List of submissions with chunks and embeddings.
 */
...
/**
 * Saves a checked document and its embeddings to MongoDB.
 * @param {string} filename - The uploaded filename.
 * @param {string} text - The raw extracted text.
 * @param {Array<{text: string, embedding: number[]}>} chunks - List of text chunks and vectors.
...
/**
 * Performs local vector similarity checks against pre-loaded database submissions in memory.
 ...
 */
export function searchSimilarityLocal(queryEmbedding, dbSubmissions, threshold = 0.40, limit = 5) {
  const matches = []
